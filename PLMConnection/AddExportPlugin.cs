...
namespace PLMConnection {
    public class AddExportPlugin : IAddinClientEntry, IAutoExec {
        ...
        private const string OPERATION_ID = "PLM32_ExportToERP";
        private const string OPERATION_LABEL = "导入ERP";
        private const string OPERATION_TOOLTIP = "导入到ERP";
        private const string OPERATION_FILTER = "PLMConnection.dll,PLMConnection.ExportToERPFilter";
        private const string OPERATION_EVENTHANDLE = "PLMConnection.dll,PLMConnection.ExportHandlerHelper,OnExportToERP";
...
,"RelationTreeChild"};
        ...
        public void Init() {
            _classes = ERPServiceHelper.Instance.ConfigData.Classes;
            #region 绑定定版后事件
                ...
                for (int i = 0; i < _classes.Count; i++) {
                    for (int j = 0; j < OBCOLLECTION_SENCE.Count; j++) {
                        for (int k = 0; k < OBCOLLECTION_MODE.Count; k++) {
                            var col = CreateDEBindingOprt(_classes[i], OBCOLLECTION_MODE[k], OBCOLLECTION_SENCE[j], cols);
            ...
            if (bizItems != null) {
                ...
                foreach (object obj2 in list) {
                    if (typeof(IBizItem).IsInstanceOfType(obj2)) {
                        ...
                        foreach (string parentClass in _classes) {
                            if (!ModelContext.MetaModel.IsChild(parentClass,bItem.ClassName)) {
                                continue;
                            }
